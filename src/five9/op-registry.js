...
export const OP_CLASSIFICATION = Object.freeze({
  ...
  addRecordToListSimple: c('write', 'skip', 'not-built', 'Still not built, but NOT for the reason recorded here until 2026-09-04 ("redundant with addRecordToList; no new capability"). It is not redundant: its listUpdateSimpleSettings is the ONLY settings type carrying callAsap, so it is the only op that could enter the Dial ASAP queue by that name. It stays unbuilt because addRecordToList reaches the same outcome through listUpdateSettings.callNowMode (NONE | NEW_CRM_ONLY | NEW_LIST_ONLY | ANY), which the shipped builder now emits for the callback push. If a future need wants callAsap or timeToCall semantics specifically, this is a real capability gap, not a duplicate.'),
