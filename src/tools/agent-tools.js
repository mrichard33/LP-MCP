...
export function registerAgentTools(server) {
  ...
  server.tool(
    ...
    'Queue an agent action for execution. Actions can auto-execute or require human approval.',
    {
      ...
      action_type: z.string().describe('Action type. Common: add_tag, remove_tag, set_stage, update_custom_fields, update_contact_email, move_opportunity, update_opportunity, add_to_workflow, remove_from_workflow, send_message, create_task, send_notification, book_appointment, reschedule_appointment, cancel_appointment, emit_event, layer3_dispatch. Five9 admin writes (five9_start_campaign, five9_stop_campaign, five9_reset_campaign, five9_set_outbound_campaign, five9_add_records_to_list, five9_delete_record_from_list, five9_add_numbers_to_dnc, five9_remove_numbers_from_dnc) MUST be queued with requires_approval: true and only execute when FIVE9_WRITES_ENABLED is set. Do NOT use update_contact — unimplemented in the executor and historically wiped tags via PUT. See LP MCP src/actions/index.js ACTION_HANDLERS for the full registry.'),
