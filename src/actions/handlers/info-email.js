 ...
 *   5. send
 *
 * TWO WAYS TO SEND (2026-09-24). One switch: INFO_EMAIL_WEBHOOK_URL.
 *   unset  → direct: the Conversations API, as above.
 *   set    → workflow: POST { subject, preheader, body_html, … } to that GHL
 *            Inbound Webhook, and the workflow sends it in the branded
 *            template. Mark's requirement: every email carries a subject, a
 *            preheader AND a body. The webhook carries all three in one
 *            request, so there is no custom field to go stale and no tag to
 *            re-arm.
 *
 * Checked against every email workflow live on 2026-09-24 — none already
 * does this:
 *   U.SEND-AI  tag-triggered; GPT-4o writes its own body; fixed subject; no preheader
 *   I.AI-MAIL  tag-triggered; GPT writes subject/preview/body into fields S4.5 shares
 *   S4.5       webhook-triggered, but it is the nurture rotation — entering it
 *              enrolls the lead in the sequence
 *   I.AG-IN    webhook-triggered reply path; subject + body, no preheader, and
 *              it reassigns the contact's user around every send
 * So the webhook URL is set only after a workflow is built (or U.SEND-AI is
 * converted) to send the three webhook fields as-is. Never point it at a
 * workflow that rewrites the text: that copy skips every check here.
 *
 * The direct path carries the preheader too, as the standard hidden preview
 * line at the top of the HTML, so both paths show the same inbox preview.
 */
...
/** INFO_EMAIL_WEBHOOK_URL set → 'workflow', else 'direct'. */
export function resolveInfoEmailDelivery(env = process.env) {
  return String(env?.INFO_EMAIL_WEBHOOK_URL || '').trim() ? 'workflow' : 'direct';
}
...
/**
 * The exact JSON the GHL Inbound Webhook receives. Field names are what the
 * workflow maps: {{inboundWebhookRequest.subject}}, .preheader, .body_html.
 */
export function buildInfoEmailWebhookPayload({ contactId, email, subject, preheader, body, actionId = null }) {
  return {
    contact_id: String(contactId),
    email,
    subject,
    preheader,
    body_html: buildInfoEmailHtml(body),
    body_text: body,
    action_id: actionId,
    source: 'lp_mcp_send_info_email',
  };
}
 ...
 *   postWebhook(payload)            → Promise (workflow mode only)
 */
export async function executeSendInfoEmail(action, _context = {}, deps = null) {
  ...
  if (delivery === 'workflow') {
    await d.postWebhook(buildInfoEmailWebhookPayload({
      contactId, email, subject, preheader, body, actionId: action.id || null,
    }));
    sent = { sendMethod: 'ghl_inbound_webhook' };
  } else {
    sent = await d.sendEmail(contactId, withPreheader(buildInfoEmailHtml(body), preheader), subject, action);
  }
  ...
  await d.emitEvent({
    ...
    entity_id: String(contactId), ghl_contact_id: String(contactId),
    priority: 'normal',
    payload: { subject, preheader, delivery, send_method: sent?.sendMethod || null, action_id: action.id || null, body_chars: body.length },
...
async function defaultDeps() {
  ...
  return {
    ...
    emitEvent,
    // rate-limiter-exempt: a GHL INBOUND webhook, not the rate-limited v2 API.
    postWebhook: async (payload) => {
      const res = await fetch(String(process.env.INFO_EMAIL_WEBHOOK_URL).trim(), {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(`info email webhook ${res.status}: ${text.slice(0, 200)}`);
      }
    },
  };
}
