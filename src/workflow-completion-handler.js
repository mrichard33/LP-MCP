 ...
 *   3. That workflow sends ONLY contactId to this endpoint
 *   4. This endpoint self-enriches, resolves the workflow, emits the event
 *   5. The GHL workflow then removes the completed:* tag (cleanup)
 * 
 * v1.0 — Initial implementation. Tag-to-workflow mapping for 16+ workflows.
 */
...
// Maintained here for now. Can move to Supabase for dynamic updates.
// ═══════════════════════════════════════════════════════════════════
...
const TAG_TO_WORKFLOW = {
  ...
  // Objection Validator (utility but needed for bridge rules)
...
/**
 * Self-enriching handler for workflow completion via tag signal.
 * 
 * Accepts just contactId, calls GHL API, finds completed:* tag,
 * maps to workflow ID, emits ghl.workflow_completed event.
 * 
 * Same event format as handleWorkflowCompleted in behavioral-emitter.js
 * so all existing Decision Engine rules work without changes.
 */
export async function handleWorkflowCompletedByTag(req, res) {
  const body = req.body || {};
  ...
  // Also accept tags from webhook body as fallback (comma-separated or array)
  if (tags.length === 0) {
  ...
  // Find completed:* tags and resolve workflows
  const completedTags = tags.filter(t => t.startsWith('completed:'));
  if (completedTags.length === 0) {
    console.warn(`[WorkflowCompletion] No completed:* tags found for ${contactId}. Tags: ${tags.slice(0, 10).join(', ')}`);
    return res.json({ status: 'accepted', event_type: 'none', reason: 'no_completed_tag_found' });
  }

  // Emit an event for each completed:* tag found (usually just one)
  const emitted = [];
  for (const tag of completedTags) {
    ...
    await emitEvent({
      ...
      payload: {
        ...
        completion_tag: tag,
        resolved_via: workflow ? 'tag_mapping' : 'unknown_tag',
      },
      priority: 'normal',
