 ...
 */
export async function syncToLp(call, summary, match, { db = supabase, cfg = getConfig(), lpClient } = {}) {
  const target = 'lp';
  ...
  const noteBody = composeNote(call, summary);
 ...
 */
export async function syncToGhl(call, summary, match, { db = supabase, cfg = getConfig(), ghlClient } = {}) {
  const target = 'ghl';
