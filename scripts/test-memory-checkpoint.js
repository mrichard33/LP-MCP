 ...
 *   seed.byKey     row returned for a lookup by checkpoint_key — a function
 *                  (ctx, n) => row|null when it has to change mid-test
    ...
    const finish = async (maybe) => {
      ...
      if (ctx.op === null && table === 'claude_session_logs' && maybe) {
        if (ctx.filters.some(([k]) => k === 'checkpoint_key')) {
          const row = typeof seed.byKey === 'function' ? seed.byKey(ctx, calls.length) : seed.byKey;
...
test('apply (new session): the session INSERT carries the deterministic checkpoint_key and the result echoes it', async () => {
  ...
  const sess = db.calls.find((c) => c.table === 'claude_session_logs' && c.op === 'insert').payload;
  assert.match(sess.checkpoint_key, /^[0-9a-f]{64}$/, 'sha256 hex, not a uuid (sql/099)');
  assert.equal(out.checkpoint_key, sess.checkpoint_key); assert.equal(out.attempts, 1); assert.equal(out.recovered, false);
  assert.equal(out.inserted, true); assert.match(out.note, /^wrote session #900$/);
  // Same identity, second call, fresh database handle: the same key.
  const again = await applyCheckpoint(base, { db: fakeDb(), retry: NO_RETRY_WAIT });
  assert.equal(again.checkpoint_key, out.checkpoint_key);
...
test('a transient failure mid-sequence is retried and the retry only writes what has not landed', async () => {
  ...
  assert.equal(byKey.length, 1, 'looked the key up once, before the insert; the retry already knew the session id');
});

test('a lost session-insert response is recovered by checkpoint_key on retry — no second session row', async () => {
  let n = 0;
  const db = fakeDb({
    // The row only exists once the (response-less) insert has run.
    byKey: () => (n > 0 ? { id: 4242 } : null),
