...
### Mechanism 2 — Timestamp backfill (automatic, chat surface only)
...
1. Query unlinked sessions: `chat_url IS NULL AND surface='chat' AND created_at > now() - interval '7 days'`.
2. Call `recent_chats(n=20)`; paginate with `before` back to the oldest unlinked `created_at`, at most 5 calls.
3. For each unlinked session, the match is the chat whose `updated_at` is within **3 minutes** of the session's `created_at`. The checkpoint reply is the chat's last message, so the gap is normally seconds. If two chats fall inside the window, pick the one whose title is closest to the session title; if still ambiguous, skip it and say so.
4. One UPDATE per match: `chat_url`, `chat_title`, `link_confidence='inferred'`, plus a ledger row `disposition='linked'`. Never overwrite an `exact` link with an `inferred` one.
5. Report one line: "Linked N sessions to their chats." Say nothing if N = 0 and nothing is unlinked.

**"Link sweep"** — when Mark says this, run only steps 1–5 (window widened to 30 days) and stop. Because `recent_chats` is scoped to the project the chat is in, a sweep across several projects needs one new chat opened in each project (and one outside any project) with this command.
...
4. **Provenance on the refresh.** Rows Mark states or confirms in the refresh
   ...
   rows Mark states now are `origin = 'live'`. Anything Claude infers from the
   transcript without Mark's word stays `retro` / `reconstructed` or goes to
   `claude_pending_items` as `unconfirmed_decision`.
...
- [ ] Auto-link pass run (Mechanism 2, mandatory): every unlinked `surface='chat'` session from the last 7 days matched to `recent_chats` by timestamp, URL + ledger row written
