...
2. **Match on search keys (primary).** For each row, call `conversation_search` with 2–3 of its most distinctive `transcript_search_keys`. Accept a chat only when the keys hit **and** its title is consistent with the session title. This works at any age, needs no pagination, and does not assume `created_at` is the real conversation time.
